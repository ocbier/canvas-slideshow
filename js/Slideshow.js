...
class Slideshow {
    constructor(slideshowElemID, imageDataJSONLoc) {
        this.images = []; //Array holding references to Image objects.
        this.globalImageWidth = Slideshow.defaultImageWidth; //Actual value for image width. Set to default initially
        this.globalImageHeight = Slideshow.defaultImageHeight; //Actual value to use for image height. Set to default initially.
        this.interval = 0; //ID returned by setInterval() when a function is assigned.
        this.imageCounter = 0; //current image index in array "images".
        this.controls = null; //ControlElements for slideshow.
     ...
     */
    preload() {
        this.getJSONData((request, thisArg) => {
    ...
    start(loadTimeoutCounter, showPreloader = true) {
    ...
    handleImageLoad(loadTimeoutCounter, showPreloader) {
    ...
    preloadImages(request, thisArg) {
        if (request.readyState === 4 && request.status === 200) {
            ...
            imageLocationList.forEach((current) => {
    ...
    */
    ...
    resizeCanvas(isFullscreen = false) {
        /*Use the computed style to set the actual width  (x coordinate range)
          for canvas. This allows dynamic styling with css. */
        let computedWidth = window.getComputedStyle(this.canvas, null)["width"];
        let computedHeight = window.getComputedStyle(this.canvas, null)["height"];
    ...
    fadeTransition(captionElem, nextCounter, width = -1, height = -1) {
        var imgWidth = width;
    ...
    /* Creates a horizontal sliding effect between slides. The overall approach here is to
       ...
       @param width Optional value for width of images to draw, which will be set to defaultWidth if null
       @param height Optional value for height of images to draw, which will be set to defaultHeight if null */
    horizontalTransition(captionElem, nextCounter, width = -1, height = -1) {
        clearInterval(this.interval);
        var imgWidth = width;
        var imgHeight = height;
        //If no valid width is specified, use globalImageWidth
        ...
        let forward = this.imageCounter < nextCounter;
    ...
    /* Produces a vertical scrolling effect to transition between images.
      ...
      difference is that translations are applied in the y-axis, rather than in the x-axis.
        ...
        @param width Optional value for width of images to draw, which will be set to defaultWidth if null
        @param height Optional value for height of images to draw, which will be set to defaultHeight if null */
    verticalTransition(captionElem, nextCounter, width = -1, height = -1) {
        clearInterval(this.interval);
        let imgWidth = width;
        ...
        let forward = this.imageCounter < nextCounter; //Is movement forward or backwards?
    ...
    */
    ...
    effectCallback(captionElem, option) {
        console.log(option);
...
/*****Slideshow settings *******/
Slideshow.defaultImageWidth = 640;
Slideshow.defaultImageHeight = 480;
Slideshow.playSpeed = 8000; //Slide show play speed in ms.
Slideshow.aspectRatio = 16 / 9; //Aspect ratio for images.
Slideshow.animationSpeed = 33; //Redraw rate for animations.
Slideshow.fadeStep = 0.008; //Amount to fade after each redraw.
Slideshow.loadTimeout = 90000; //Timeout for image loading set to 90000ms or 1m 30s.
Slideshow.preloaderElemID = "preloader";
...
class ControlElement {
    constructor(elem, enabled, callback, eventName) {
...
class ControlsContainer {
    ...
    constructor(playButton, directionButton, randomButton, nextButton, previousButton, effectSelector, fullScreenButton) {
